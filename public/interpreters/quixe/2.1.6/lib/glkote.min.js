...
var cursel=$('#win'+win.id+'_cursor',dom_context);if(cursel.length){cursel.parent().removeClass('InputLine');cursel.remove();}
cursel=null;if(arg.clear){win.frameel.empty();win.topunseen=0;win.pagefrommark=0;}
if(text===undefined)
...
var divel=buffer_last_line(win);if(divel){cursel=$('<span>',{id:'win'+win.id+'_cursor','class':'InvisibleCursor'});divel.append(cursel).addClass('InputLine');if(win.inputel){cursel.append(win.inputel);}}}
...
if(win.type=='buffer'){var cursel=$('#win'+win.id+'_cursor',dom_context);if(!cursel.length){cursel=$('<span>',{id:'win'+win.id+'_cursor','class':'InvisibleCursor'});win.frameel.append(cursel).addClass('InputLine');}
...
return null;if(!$(divel).hasClass('BufferLine'))
